...
class PNC(torch.nn.Module):
    ...
    def __init__(self, dataset, input_shape: torch.Tensor, num_outputs: int, pooling_size=(16, 12),
        ...
        self.mlp1 = Sequential(Linear(n[0]+3, n[1]),
                              ReLU(),
                              Linear(n[1], n[1]))
        self.conv1 = PointNetConv(self.mlp1)
        self.norm1 = BatchNorm(in_channels=n[1])
        self.mlp2 = Sequential(Linear(n[1]+3, n[2]),
                              ReLU(),
                              Linear(n[2], n[2]))
        self.conv2 = PointNetConv(self.mlp2)
        self.norm2 = BatchNorm(in_channels=n[2])

        self.mlp3 = Sequential(Linear(n[2]+3, n[3]),
                              ReLU(),
                              Linear(n[3], n[3]))
        self.conv3 = PointNetConv(self.mlp3)
        self.norm3 = BatchNorm(in_channels=n[3])
        self.mlp4 = Sequential(Linear(n[3]+3, n[4]),
                              ReLU(),
                              Linear(n[4], n[4]))
        self.conv4 = PointNetConv(self.mlp4)
        self.norm4 = BatchNorm(in_channels=n[4])

        self.mlp5 = Sequential(Linear(n[4]+3, n[5]),
                              ReLU(),
                              Linear(n[5], n[5]))
        self.conv5 = PointNetConv(self.mlp5)
        self.norm5 = BatchNorm(in_channels=n[5])
        self.pool5 = MaxPooling(pooling_size, transform=Cartesian(norm=True, cat=False))

        self.mlp6 = Sequential(Linear(n[5]+3, n[6]),
                              ReLU(),
                              Linear(n[6], n[6]))
        self.conv6 = PointNetConv(self.mlp6)
        self.norm6 = BatchNorm(in_channels=n[6])
        self.mlp7 = Sequential(Linear(n[6]+3, n[7]),
                              ReLU(),
                              Linear(n[7], n[7]))
        self.conv7 = PointNetConv(self.mlp7)
